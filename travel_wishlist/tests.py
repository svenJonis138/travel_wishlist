...
class TestHomePage(TestCase):
    def test_home_page_shows_empty_list_msg_for_empty_db(self):
...
class TestVisitedPage(TestCase):
    ...
    def test_visited_page_shows_empty_list_message_with_empty_database(self):
...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        new_place_data = {'name': 'St Paul', 'visited': False}
        response = self.client.post(add_place_url, new_place_data, follow=True)
        self.assertTemplateUsed(response, 'travel_wishlist/wishlist.html')

        response_places = response.context['places']
        self.assertEqual(1, len(response_places))  # check that only one place was added
        st_paul_from_response = response_places[0]
        st_paul_from_database = Place.objects.get(name='St Paul', visited=False)
        self.assertEqual(st_paul_from_database, st_paul_from_response)
...
class TestVisitPlace(TestCase):
    ...
    def test_non_existent_place(self):
        visit_nonexistent_place_url = reverse('place_was_visited', args=(1138, ))
